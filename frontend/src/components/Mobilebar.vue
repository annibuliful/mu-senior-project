<template>
  <div :class="classMenuIcon">
    <img
      src="@/assets/icons/menu.svg"
      class="ml-auto img-menu relative mr-2 p-2 overflow-x-scroll "
      @click="onClickMenu"
    />
...
export default {
  data() {
    return {
      isClicked: false,
      classMenuIcon:
        "flex flex-col h-screen lg:hidden md:hidden sm:visible fixed z-10"
    };
  },
  props: ["isCancelLink"],
  methods: {
    ...
    onClickMenu() {
      ...
      if (this.isClicked) {
        this.classMenuIcon =
          "flex flex-col h-screen lg:hidden md:hidden sm:visible fixed z-10 bg-orange-300";
      } else {
        this.classMenuIcon =
          "flex flex-col h-screen lg:hidden md:hidden sm:visible fixed z-10";
      }
