  ...
  <div>
    <div class="flex flex-row py-4" v-if="sortBy == 'name'">
      ...
      <div class="flex flex-col w-3/4">
        <div class="ml-2 mt-2">{{ vaccines[0] }}</div>
        <div class=" ml-2">{{ dateFormat }}</div>
        <div class="ml-2 text-sm">{{ childname }}</div>
      </div>
    </div>

    <div class="flex flex-row" v-if="sortBy == 'date'">date</div>
  </div>
</template>
<script>
...
export default {
  created() {
    if (this.status == "in-progress") {
      this.classVerticleByStatus += " bg-gray-300";
    } else if (this.status == "vaccinated") {
      this.classVerticleByStatus += " bg-green-300";
    } else if (this.status == "vaccinating") {
      this.classVerticleByStatus += " bg-yellow-300";
    } else if (this.status == "overdue") {
      this.classVerticleByStatus += " bg-red-300";
    }
  },
  data() {
    return {
